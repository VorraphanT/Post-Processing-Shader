...
float dither(float x, float2 uv) {
    ...
    float2 center = fDotSize * (floor(pixelPos / fDotSize) + 0.5);
    float2 offset = pixelPos - center;
    float dist = length(offset);
    float maxRadius = fDotSize * 0.5;

    float threshold = dist / maxRadius;
